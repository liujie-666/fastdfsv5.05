...
RUN /bin/sh -c "mkdir -p ${FASTDFS_PATH}/libfastcommon  && mkdir -p ${FASTDFS_PATH}/fastdfs-5.05  && mkdir ${FASTDFS_BASE_PATH}"
...
WORKDIR /opt/fdfs/fastdfs-5.05
# 这里需要修改为add fastdfs
ADD source/fastdfs-5.05.tar.gz ${FASTDFS_PATH}
RUN ./make.sh  && ./make.sh install  && rm -rf ${FASTDFS_PATH}/fastdfs-5.05
